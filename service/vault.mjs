...
import {sha,run,need,atomic} from './common.mjs';
...
export async function storeSecrets(c,value) {
  ...
  if(process.platform==='win32') {
    const r=await run(c.powershell,['-NoProfile','-NonInteractive','-Command',"$s=[Console]::In.ReadToEnd() | ConvertTo-SecureString -AsPlainText -Force; [Console]::Write(($s | ConvertFrom-SecureString))"],{input:body});
...
export async function loadSecrets(c) {
  ...
  if(process.platform==='win32') {
    ...
    const r=await run(c.powershell,['-NoProfile','-NonInteractive','-Command',"$s=ConvertTo-SecureString ([Console]::In.ReadToEnd()); [Console]::Write(([System.Net.NetworkCredential]::new('', $s)).Password)"],{input:fs.readFileSync(file,'utf8')});
